...
def readIMU(q, b, fake_online_data, init_time, signals_per_sensor, save_dir_init,home_dir):
    ...
    with open(home_dir+'settings.txt', 'r') as f:
        for cnt, line in enumerate(f): #cnt is line number
    ...
    # define sensors
    sensor_inds = tca_inds[1:] #note that tca_inds[0] in the paper is the i2c button
    ...
    for i, s_ind in enumerate(sensor_inds):
        if s_ind != 9: #body part not being used if its 9
            if not fake_real_time:
                if alt_address_list[i]: # if True, settings.txt file, number as a b inside -> alternate i2c address
    ...
    if not fake_real_time: #fake_real_time is False so look for the not
        ...
        if calibrate_sensors or not os.path.exists(cal_dir): # also check if calibration folder exists, else create dir and calibrate
            ...
            print("Button off -> Gyro Offset Done")
