...
#Define the GPIOs
BUTTON  = board.D21
LED     = board.D20 #remember to wire this part
...
def readIMU(q, b, fake_online_data, init_time, signals_per_sensor, save_dir_init,home_dir):
    ...
    if not fake_real_time:
        #I think the trigger is optional
        ...
        # Initializing the button and LED, and the TCA channel
        ...
        button_mode(led,0) # turn led off
        clear_button(button) #not too sure about this
    ...
    if not fake_real_time:
        ...
        if calibrate_sensors or not os.path.exists(cal_dir): # also check if calibration folder exists
            ...
            print("Calibration Done!") #might need a non Serial way of showing the calibraiton
            button_mode(led, 0) # turn button off
    ...
    if fake_real_time:
        ...
        while(True): # outer loop for resetting the simulation
            button_mode(led, 1) # make button blink
            clear_button(button)
            ...
            start = q.get() # waiting for confirmation of sim Starting
            time.sleep(0.3)
            button_mode(led, 2) # make button solid red to start recording
            clear_button(button)
...
def button_mode(led,state):
    if state == 0: # turn off
        led.value = False
    elif state == 1: # blink
        led.value = not led.value #alternate between different states
...
def check_button(button, last_pressed, time_min = 1.0):
    """
    Debouncing
    Check for button press, must be pressed at least time_min apart before it can be pressed
    state refers to whether it has been pressed or not
    """
    ...
    if (time.time() - last_pressed > time_min) and (button.value != False):
        state = True
        last_pressed = time.time()
