...
save_dir = save_dir_init+save_folder+'/poc' # append the folder name here -> change name to /poc to save data there
    ...
    while(running):
        if (b.qsize() > 0) or t == sim_steps: # new button press so we should save the data and restart the sim
            ...
            if log_data:
                ...
                osim.STOFileAdapter.write(ik_results, save_dir+save_file+str(file_cnt)+'.sto') #Need to make sure that the model writes correctly.
                if log_temp and not fake_real_time:
